"""This script takes parameters <dirpath> and  a count where the 
directory is assumed to contain image files and an _annotations.coco.json 
file. This script splits dataset into two datasets containing count 
images in one dataset and total-count images in the other. Each of these 
new datasets will have their own _annotations.coco.json file. The two datasets are
placed in directories called 
    <dirname>_1_<count> 
    <dirname>_2_<total - percent>"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(f"Call is: python {sys.argv[0]} <dataset dir> count")
        sys.exit(0)
    ...
    # Create directories
    dir1_path = src_dir + "_1_" + str(count)
    dir2_path = src_dir + "_2_" + str(total-count)
